...
knitr::opts_chunk$set(echo = TRUE, warning=FALSE, comment=NA, message=FALSE, fig.path="figures/", dpi=600)
```
...
If package installation with `install_github()` fails, try to download the package using an alternative method (i.e. wget), such as:
...
**If you encounter a bug or if you have any problems, please file an [issue](https://github.com/RS-eco/geodat/issues) on Github.**
...
```{r, marrealm_map, fig.width=10, fig.height=7}
...
```{r, zoorealm_map, fig.width=10, fig.height=4}
library(sf)
